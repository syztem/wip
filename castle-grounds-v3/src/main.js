...
const marioPose = { x: 0, z: 16.2, yaw: Math.PI, moving: false };
let lastShot = '';
let flashUntil = 0;
let nextBolt = 2.4;
...
renderer.setAnimationLoop(() => {
  ...
  const tgt = chapter.mario;
  marioPose.x += (tgt.x - marioPose.x) * Math.min(1, dt * 1.8);
  marioPose.z += (tgt.z - marioPose.z) * Math.min(1, dt * 1.8);
  let dyaw = tgt.yaw - marioPose.yaw;
  while (dyaw > Math.PI) dyaw -= Math.PI * 2;
  while (dyaw < -Math.PI) dyaw += Math.PI * 2;
  marioPose.yaw += dyaw * Math.min(1, dt * 2.4);
  marioPose.moving = tgt.moving;
  mario.update(u, bridge, marioPose);

  if (trees) trees.update(t, world);
  if (props) props.update(t, world);
  if (castle.nave) castle.nave.update(t);
