...
export function createCastle(env) {
  ...
  const stone = new THREE.MeshStandardMaterial({ map: stoneMap(), roughness: 0.78, metalness: 0.02, color: 0xf3e6cc });
  const stoneDark = new THREE.MeshStandardMaterial({ color: 0x6a5844, roughness: 0.9, metalness: 0 });
  const roof = new THREE.MeshStandardMaterial({ map: roofMap(), roughness: 0.55, metalness: 0.04, color: 0xd03030 });
  const gold = new THREE.MeshStandardMaterial({ color: 0xe6c35c, roughness: 0.28, metalness: 0.65 });
  const glass = new THREE.MeshPhysicalMaterial({
    map: glassMap(), color: 0xffffff, metalness: 0, roughness: 0.08,
    transmission: 0.82, thickness: 0.35, ior: 1.5, transparent: true,
    side: THREE.DoubleSide,
    attenuationColor: new THREE.Color().setHex(0xc45a88, THREE.SRGBColorSpace),
    attenuationDistance: 0.8, envMap: env || null, envMapIntensity: 1.1,
  });
  ...
  for (const [tx, tz] of [[-12.5, -9], [12.5, -9], [-12.5, 9], [12.5, 9]]) {
  ...
  for (let s = 0; s < 6; s++) box(stone, 8 - s * 0.35, 0.38, 1.15, 0, 0.25 + s * 0.38, 14.2 + s * 0.55, root);
  ...
  for (const [wx, wy, wz] of [[-8, 10, 11.15], [8, 10, 11.15], [-8, 10, -11.15], [8, 10, -11.15], [0, 24, 4.1]]) {
